...
const bnbPools = poolsConfig[chainId].filter((p) => p.stakingTokenName === quoteTokenBNB)
const nonMasterPools = poolsConfig[chainId].filter((p) => p.sousId !== 0)
...
export const fetchUserPendingRewards = async (account) => {
  ...
  // Cake / Cake pool
  const pendingReward = await masterChefContract.methods.pendingSatoshi('0', account).call()
