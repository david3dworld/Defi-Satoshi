...
import { AutoRenewIcon, Box, Button, Flex, Text } from '@satoshicrypto/uikit'
...
const Claim: React.FC<ClaimProps> = ({
  ...
  const contributedBalance = getBalanceNumber(userInfo.amount, 'SAT')
  const refundedBalance = getBalanceNumber(refundingAmount, 'SAT').toFixed(userInfo.amount.eq(0) ? 0 : DISPLAY_DECIMALS)
  const { tokenSymbol, tokenDecimals } = ifo
  const rewardBalance = getBalanceNumber(offeringTokenBalance, tokenSymbol)
