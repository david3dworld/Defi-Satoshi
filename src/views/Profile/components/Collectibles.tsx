...
import { Heading, Text, Flex, ChevronRightIcon } from '@satoshicrypto/uikit'
import { Link } from 'react-router-dom'
import useI18n from 'hooks/useI18n'
import { nfts } from 'config/constants'
...
const Collectibles = () => {
  ...
  const nftsInWallet = nfts[chainId].filter((nft) => bunnyIds.includes(nft.bunnyId))

  return (
    <>
      <Heading as="h4" size="md" mb="8px">
        {TranslateString(999, 'Satoshicrypto Collectibles')}
      </Heading>
      <Text as="p">
        {TranslateString(
          999,
          'Satoshicrypto Collectibles are special ERC-721 NFTs that can be used on the Satoshi platform.',
        )}
      </Text>
      <Text as="p">
        {TranslateString(
          999,
          "NFTs in this user's wallet that aren't approved Satoshicrypto Collectibles won't be shown here.",
        )}
      </Text>
